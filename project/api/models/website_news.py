from sqlalchemy import Column, String, Integer, ARRAY, TIMESTAMP, BigInteger, TEXT

from .base import db, BaseColumns


class WebsiteNews(db.Model, BaseColumns):
    __tablename__ = "website_news"

    def __init__(self, **kwargs):
        for attr in ['url', 'effective_url', 'domain', 'domain_id', 'city_code', 'region_code', 'ip', 'ip_area',
                     'title', 'content', 'web_type', 'host_dept', 'host_type', 'industries', 'ai_industries', 'tags',
                     'code_language', 'http_status', 'http_status_list', 'category', 'source']:
    ...
    source = Column(String(30), server_default='', index=True)  # 来源
